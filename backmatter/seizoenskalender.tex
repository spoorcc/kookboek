...
staat de volledige lijst erbij. Elke balk hieronder loopt van januari (links)
tot december (rechts); het opgevulde stuk is het seizoen.
...
\newcommand{\seizoenbalk}[1]{%
  \begin{tikzpicture}[x=0.75cm, y=0.045cm]
    ...
    \foreach \start/\eind in {#1} {
      \draw[line width=3.4pt, color=ink, line cap=round]
        (\start-1+0.11,0) -- (\eind-0.11,0);
    }
  \end{tikzpicture}}
\newcommand{\seizoenrij}[2]{#1 & \seizoenbalk{#2} \\[0.3em]}
\newcommand{\seizoenkop}{%
  & \seizoenmaanden \\[2pt]
  \multicolumn{2}{@{}l@{}}{\textcolor{terracotta}{\rule{\linewidth}{0.4pt}}} \\[0.3em]}
...
\seizoenrij{Doperwt}{5/7}
\seizoenrij{Komkommer}{7/10}
\seizoenrij{Ma\"is}{8/9}
...
% \seizoenrij{Witte kool}{1/3,7/12}
