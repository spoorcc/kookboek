...
- **Register entries**: match existing singular/plural and spelling choices (`Tomaten` not `Tomaat`, `Maïs` with the trema, `Kipdijfilet` when the ingredient specifically is kipdijfilet rather than the generic `Kip`) so the same ingredient always collects under one Register heading instead of splitting across near-duplicates.
