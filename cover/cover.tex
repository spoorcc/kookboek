...
%  generated for a 164-page interior). \spinew has since been updated for
%  the 208-page interior using the guide p14 hardcover spine-width table
%  (the same table scripts/lulu_lint.py checks against), not a freshly
%  regenerated Lulu template — reconfirm it against the real tool before
%  ordering.
%
...
\newcommand{\interiorpagecount}{208} % <-- UPDATE to match the built interior PDF
\newlength\spinew
\setlength\spinew{19mm} % <-- guide p14 hardcover spine-width table bucket for
% 208 pages (195-222 pages -> 19mm); reconfirm against a regenerated Lulu
% Project Creation Tool template before ordering a real proof (see note above)
