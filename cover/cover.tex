...
%
%  Unlike a paperback, Lulu does not publish a spine-width formula for
%  casewrap covers — spine width comes from an internal lookup table keyed
%  on page count, and is only known for certain once you generate a cover
%  template from Lulu's own Project Creation Tool after uploading the
%  interior file. Trim, wrap area and safety margin below were read
%  directly off one such template, q6qdvpe-cover-template.pdf (originally
%  generated for a 164-page interior). \spinew was last confirmed against a
%  freshly regenerated Lulu template for a 223-page interior (0.813in /
%  20.65mm spine). The interior is now 231 pages, still inside that same
%  guide p14 lookup-table bucket (223-250 pages -> nominal 21mm), so
%  \spinew is left at the last real-tool reading rather than guessed at —
%  but that reading was for 223 pages specifically, not 231, so treat it as
%  an estimate and reconfirm it against the real tool again before ordering.
%
...
%   - Re-download the cover template from Lulu's Project Creation Tool for
%     the current page count and confirm \spinew below still matches its
%     reported spine width — don't assume the same lookup-table bucket
%     still applies.
% =============================================================================
...
\newcommand{\interiorpagecount}{231} % <-- UPDATE to match the built interior PDF
\newlength\spinew
\setlength\spinew{20.65mm} % <-- 0.813in, last real-tool reading (for a 223-page
% interior, q6qdvpe-cover-template.pdf); interior is now 231 pages, still the
% same guide p14 bucket (223-250 -> nominal 21mm) so this is kept as the
% closest known estimate, but it is NOT a fresh reading for 231 pages —
% re-download the cover template from Lulu's Project Creation Tool and
% confirm before ordering a real proof
