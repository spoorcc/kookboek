...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Spans the grid's own width, matching the mat framing it. The block's
  % five pieces used to be five independently-positioned nodes spread
  % across a wide, loosely-spaced span (needing 4 of the panel's 6 rows,
  % most of it wasted white space) — combined into one tightly-spaced
  % node below, sized to fit 2 rows instead, so the plate hides far fewer
  % of the mosaic's photos.
  \fill[paper, opacity=0.9, rounded corners=2pt]
    ([shift={(\backgridleft,\dimexpr\gridtop-3\backch\relax)}]current page.south west) rectangle
    ([shift={(\backgridright,\dimexpr\gridtop-1\backch\relax)}]current page.south west);

  \node[anchor=center, align=center, text width=\boxw] at
      ([shift={(\backboxcenterx,\dimexpr\gridtop-2\backch\relax)}]current page.south west) {%
    ...
    Proef vooral mee, en pas gerust aan waar nodig.}\par
    \vspace{2.5mm}
    \raisebox{0.1ex}{\textcolor{terracotta}{$\scriptstyle\blacklozenge$}}\par
    \vspace{1.5mm}
