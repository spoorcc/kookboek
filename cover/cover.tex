...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % Double rule (a bold outer line plus a thin inner one, inset within
  % the mat) instead of a single flat line — a classic framed-print
  % treatment that reads less like a plain box round the edge.
  \newcommand{\drawframe}[2]{%
    \draw[ink, line width=1.3pt]
      ([shift={(#1,\boxbottom)}]current page.south west) rectangle
      ([shift={(#2,\boxtop)}]current page.south west);
    \draw[ink, line width=0.5pt]
      ([shift={(\dimexpr#1+2.5mm\relax,\dimexpr\boxbottom+2.5mm\relax)}]current page.south west) rectangle
      ([shift={(\dimexpr#2-2.5mm\relax,\dimexpr\boxtop-2.5mm\relax)}]current page.south west);
  }
  \drawframe{\frontboxleft}{\frontboxright}
  ...
  \drawframe{\backboxleft}{\backboxright}
