...
\usepackage{tikz}
\usepackage{graphicx}
\usepackage{qrcode} % back-cover QR code linking to the online recipe site
...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % whole grid-row boundaries (in units of \backch down from \gridtop),
  ...
  % Spans the grid's own width, matching the mat framing it. Sized to 3
  % rows: the quote was expanded with a bit more of the voorwoord's own
  % context (who it's for, that nothing here is invented, that this is
  % the first time it's all been written down), so the tight 2-row plate
  % from before no longer had room for it without crowding.
  \fill[paper, opacity=0.9, rounded corners=2pt]
    ([shift={(\backgridleft,\dimexpr\gridtop-4\backch\relax)}]current page.south west) rectangle
    ([shift={(\backgridright,\dimexpr\gridtop-1\backch\relax)}]current page.south west);
  ...
  % Standalone QR code, bottom-left of the safety box, the way a
  % publisher's barcode/website code sits on a real book's back cover —
  % separate from the "Over dit boek" copy above, not part of it. Inset
  % past \cornerarm so it doesn't collide with the diamond corner
  % ornament. One \colorbox{paper} wraps the whole label (code + caption)
  % as a single opaque tile: qrcode's own "white" modules are drawn as
  % zero-width (i.e. genuinely transparent) rules, not opaque ink, so
  % without a real backing the mosaic underneath would bleed straight
  % through the code itself, right where a scanner needs clean contrast
  % most.
  \node[anchor=south west, inner sep=0pt] at
      ([shift={(\dimexpr\backboxleft+1.4\cornerarm\relax,\dimexpr\boxbottom+1.1\cornerarm\relax)}]current page.south west) {%
    \colorbox{paper}{\begin{minipage}[c]{3.1cm}\centering
      \vspace{2mm}
      {\color{ink}\qrcode[height=1.9cm,padding]{https://spoorcc.github.io/kookboek}}\par
      \vspace{1.5mm}
      {\fontsize{6}{7.5}\selectfont\color{terracotta}\labelcaps{4.0}{Recepten online}}\par
      {\fontsize{5.5}{7}\selectfont\color{inkfaint}spoorcc.github.io/kookboek}\par
      \vspace{2mm}
    \end{minipage}}%
  };
\end{tikzpicture}
