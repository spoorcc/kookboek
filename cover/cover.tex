...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % quote, signature and QR code) so it reads clearly over the mosaic.
  % Snapped to whole grid-row boundaries (in units of \backch down from
  % \gridtop), same reasoning as the front panel's plates: a plate edge
  % landing mid-row would leave a sliced-off strip of a photo showing
  % through. Spans the grid's own width, matching the mat framing it.
  % Sized to 4 rows: 3 for the quote (expanded with a bit more of the
  % voorwoord's own context) plus one more to fit the QR code linking to
  % the online recipe site without crowding either.
  \fill[paper, opacity=0.9, rounded corners=2pt]
    ([shift={(\backgridleft,\dimexpr\gridtop-5\backch\relax)}]current page.south west) rectangle
    ([shift={(\backgridright,\dimexpr\gridtop-1\backch\relax)}]current page.south west);

  \node[anchor=center, align=center, text width=\boxw] at
      ([shift={(\backboxcenterx,\dimexpr\gridtop-3\backch\relax)}]current page.south west) {%
    ...
    \vspace{3mm}
    % qrcode's own "white" modules are drawn as zero-width (i.e.
    % genuinely transparent) rules, not opaque white ones, so without
    % its own backing the semi-transparent paper plate would let the
    % mosaic bleed through even inside the code itself, right where a
    % scanner needs clean contrast most. \colorbox{paper} gives it a
    % fully opaque white tile of its own, quiet zone included.
    \colorbox{paper}{\color{ink}\qrcode[height=1.9cm,padding]{https://spoorcc.github.io/kookboek}}\par
    \vspace{1.5mm}
    {\fontsize{6}{7.5}\selectfont\color{terracotta}\labelcaps{4.5}{Recepten online}}\par
    {\fontsize{6}{7.5}\selectfont\color{inkfaint}spoorcc.github.io/kookboek}\par};
\end{tikzpicture}
\end{document}
