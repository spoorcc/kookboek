...
\begin{tikzpicture}[remember picture, overlay]
  ...
  % images/pasta.jpg is 1583x2000px. Printed full-bleed across the whole
  % front panel (192x252mm) it falls to ~200dpi, below Lulu's 300dpi
  % minimum. Sized as large as it can go without crowding the title
  % block above (bottom edge ~18.28cm) or the footer line below (top
  % edge ~2.45cm) — a ~2mm gap on each side — this comes out to ~326dpi.
  % Getting closer to the 300dpi floor would mean moving the title or
  % footer text, not just resizing the photo.
  \node[inner sep=0, outer sep=0, anchor=south] at
      ([shift={(\frontx+0.5\panelw,2.66cm)}]current page.south west) {%
    \includegraphics[height=15.4cm]{images/pasta}};
