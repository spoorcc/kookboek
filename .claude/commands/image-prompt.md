...
3. **Determine one page or two.** This decides which prompt(s) to produce, so get it right before writing anything. Always determine this from the recipe's own `.tex` source — never from the built PDF (`KookboekFamilieSpoor.pdf`), even if it exists and even if the recipe already has a `\heroimagefade{...}` call. Count words in the recipe's raw `.tex` file (`wc -w recipes/foo.tex`, or an equivalent whitespace-split token count — the LaTeX markup itself is fine to include, no need to strip it first for this count).
     - **≥ 200 words → two-page case.** Once a full-width hero image is present, that's reliably enough text to push a recipe to a second page. Calibrated against every recipe in this book that currently has a hero image: every one-page example tops out at 161 words, every two-page example starts at 256 — 200 sits cleanly in that gap. (As independent supporting evidence: recipes with no hero at all still spill to two pages from text length alone once they pass roughly 370 words — e.g. `lahmacun` at 412, `turkse-pide` at 480, `pizza` at 554 — while the longest current one-page, no-hero recipe sits at 332. That's consistent with, and well above, the 200-word cutoff, not a separate threshold to apply.)
     - **< 200 words → one-page case.**
     - This applies the same way whether or not the recipe already has a `\heroimagefade` call — the word count is of the recipe's own prose (title/kicker/meta/intro/ingredients/steps/tip), not of the image, so the same threshold holds either way.
   ...
   - If you can't get a word count (recipe file missing or unreadable), ask the user directly rather than guessing blind.
